...
router.post(
  ...
  (req, res) => {
    ...
    if (req.body.instagram) eventFields.eventsocial.instagram = req.body.instagram;
    eventFields.timeline = {};
    if (req.body.RegistrationDeadline) eventFields.timeline.RegistrationDeadline = req.body.RegistrationDeadline;
    if (req.body.StartingDate) eventFields.timeline.StartingDate = req.body.StartingDate;
    if (req.body.StartingTime) eventFields.timeline.StartingTime = req.body.StartingTime;
    if (req.body.EndingDate) eventFields.timeline.EndingDate = req.body.EndingDate;
    if (req.body.EndingTime) eventFields.timeline.EndingTime = req.body.EndingTime;
    if (req.body.ResultDate) eventFields.timeline.ResultDate = req.body.ResultDate;
...
// @desc    Delete event
// @access  Private
router.post('/prizes/:id',passport.authenticate('organiser',{session:false}),(req,res)=>{
 /* const{errors,isValid}=validateEducationInput(req.body);

  if(!isValid){
      return res.status(400).json(errors)
  }*/
  Organiser.findOne({user:req.user.id})
  .then(organiser=>{Event.findById(req.params.id).then(event =>{
      const newprizes={
...
router.post('/faq/:id',passport.authenticate('organiser',{session:false}),(req,res)=>{
  /* const{errors,isValid}=validateEducationInput(req.body);
 ...
 router.post('/contact/:id',passport.authenticate('organiser',{session:false}),(req,res)=>{
  /* const{errors,isValid}=validateEducationInput(req.body);
 ...
 router.post('/sponsor/:id',passport.authenticate('organiser',{session:false}),(req,res)=>{
  /* const{errors,isValid}=validateEducationInput(req.body);
